...
module SW
  class ProgressBarBasic
    ...
    def initialize(&block)
      ...
      @bar_location = [0, 35]  # relative to @location
      @bar_width = 0
      @bar_height = 10
      @fg_color = Sketchup::Color.new(120, 120, 200)
      @bg_color = Sketchup::Color.new(210, 210, 210)
      @text_location = [0, 8] # relative to @location
      @text_options = {:size => 13, :color => [80, 80, 80]} 

      # set width (causes the frame outline to be calculate)
      self.width = 300
      @lookaway = true
    ...
    # Redraw the progress bar and calculate the average redraw delay.
    ...
    # important if @lookaway is true but, on the other hand, if @lookaway is
    ...
    # @valid is set to false to indicate theat the outline needs to be recalculated
    def invalidate()
      @valid = false
    end
    ...
    def look_away()
      ...
      unless bounds.empty?
        ...
        camera.set([1000, 0, 0], [2000, 0, 0] , @up) # look into empty space
      end
    end
    private :look_away 
